...
emailInput.addEventListener('blur' , (e) => blurFromInput(
...
const submitSignupData = async function (e) {
   ...
   try {
      ...
      if (!isValidEmail(emailInput)) {errPrint(emailInput, emailErrSection, ERROR_TYPE.EMAIL_IS_NOT_VALID); signupValid = false;}; // 이메일 형식 안맞음!
      if (!isValidPw(passInput)) {errPrint(pwInput, pwErrSection,ERROR_TYPE.PW_IS_NOT_VALID); signupValid = false;}; // 패스워드 형식이 맞지 않음!!
      ...
      if (!emailInput) {errPrint(emailInput, emailErrSection, ERROR_TYPE.EMAIL_SECTION_BLANK); signupValid = false;}; // 이메일 없음!
      if (!passInput) {errPrint(pwInput, pwErrSection,ERROR_TYPE.PW_SECTION_BLANK); signupValid = false;}; // 패스워드 없음!
      if (!passCheckInput) {errPrint(pwCheckInput, pwCheckErrSection,ERROR_TYPE.PW_SECTION_BLANK); signupValid = false;}; // 패스워드 체크 없음!
      if (!isThisEmailNotOccupied) {throw new Error();}
   } catch {
      errPrint(emailInput, emailErrSection, ERROR_TYPE.EMAIL_IS_ALREADY_REGISTERED); signupValid = false; // 이미 사용중인 이메일!   
      signupValid = false;
   }
