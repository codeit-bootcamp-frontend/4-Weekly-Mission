const idInput = document.querySelector('#inputId');
const passwordInput = document.querySelector('#inputPassword');
const confirmInput = document.querySelector('#inputConfirm');
const idalert = document.querySelector('.idAlert');
const passwordalert = document.querySelector('.passwordAlert');
const confirmalert = document.querySelector('.confirmAlert');
const passwordicon = document.querySelector('#passwordIcon');
const confirmicon = document.querySelector('#confirmIcon');
const corrid = 'test@codeit.com';
const corrpassword='codeit101';
const isemail = /^([0-9a-zA-Z_\.-]+)@([0-9a-zA-Z_-]+)(\.[0-9a-zA-Z_-]+){1,2}$/;
const ispassword = /^(?=.*[a-zA-Z])(?=.*[0-9]).{8,25}$/;

let emailvalid = false;
let passwordvalid = false;
let confirmvalid = false;
const inputinfo = {};
const successInfo = {};
function signin(){
    inputinfo.email = idInput.value;
    inputinfo.password = passwordInput.value;

    fetch('https://bootcamp-api.codeit.kr/api/sign-in',{
        method: 'POST',
        headers: {
            'Content-Type': 'application/json', 
        },
        body: JSON.stringify(inputinfo),
    })
    .then(response => {
        const validresponse = response.status == 200 ? true : false;
        return response.json().then(data => ({
            ok: validresponse,
            data: data,
        }))
    })
    .then(result => {
        if (result.ok) { 
            successInfo.accessToken = result.data.data.accessToken;
            successInfo.refreshToken = result.data.data.refreshToken;
            localStorage.setItem('accessToken',successInfo.accessToken);
            // 여기부턴 이스터에그 코드
            const mainContainer = document.querySelector('main');
            const loginContainer = document.querySelector('.login');
            loginContainer.style.display='none';
            const imgteam = document.createElement('img');
            imgteam.src = './img/11팀.jpg';
            imgteam.style.width='300px';
            imgteam.style.height='420px';
            imgteam.style.zIndex='2';
            mainContainer.appendChild(imgteam);
            setTimeout(()=>{
                alert('멘토님 고생하셨어요! 다른팀 가서도 저희 잊지말고 스터디 가끔이라도 꼭 봐주셔야해요ㅎㅎㅠㅠㅠ');
            },1000);
            
            setTimeout(()=>{
                window.location.href='./shared.html';
            },5000);
            
        } else {
            const failInfo = result.data.error.message; 
            if(failInfo == 'Invalid login credentials'){
                idalert.textContent ='올바른 이메일을 입력하세요';
                passwordalert.textContent = '올바른 비밀번호를 입력하세요'
            }
            idInput.style.borderColor = '#FF5B56';
            passwordInput.style.borderColor = '#FF5B56';
        }
    })
    
}
function signup(){
    
    inputinfo.email = idInput.value;
    inputinfo.password = passwordInput.value;
    
    fetch('https://bootcamp-api.codeit.kr/api/sign-up', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json', 
        },
        body: JSON.stringify(inputinfo),
    })
    .then(response => {
        const validresponse = response.status == 200 ? true : false;
        return response.json().then(data => ({
            ok: validresponse,
            data: data,
        }))
    }) 
    .then(result => {
        if (result.ok) { 
            successInfo.accessToken = result.data.data.accessToken;
            successInfo.refreshToken = result.data.data.refreshToken;
            localStorage.setItem('accessToken',successInfo.accessToken);
            window.location.href='./shared.html';
        } else {
            const failInfo = result.data.error.message; 
            if(failInfo == 'User already registered'){
                idalert.textContent ='이미 사용 중인 이메일입니다.';
            }
            else if( failInfo == '올바른 이메일이 아닙니다.'){
                idalert.textContent = '올바른 이메일 주소가 아닙니다.';
            }
            
            idInput.style.borderColor = '#FF5B56';
        }
    })
    
}
function idfocusin(){
    idInput.style.borderColor='#6D6AFE';
}
function idfocusout(){
    if(!idInput.value){
        idalert.textContent='이메일을 입력해 주세요.';
        idInput.style.borderColor='#FF5B56'
        return;
    }

    // if(idInput.value == corrid){
    //     idalert.textContent ='이미 사용 중인 이메일입니다.';
    //     idInput.style.borderColor='#FF5B56'
    //     console.log(idInput.value);
    //     return;
    // }
    
    if(!isemail.test(idInput.value)){
        idalert.textContent = '올바른 이메일 주소가 아닙니다.';
        idInput.style.borderColor='#FF5B56'
        return;
    }
    else{
        idalert.textContent='';
        idInput.style.borderColor='#6D6AFE';
        emailvalid=true;
    }
    
}
function passwordfocusin(){
    passwordInput.style.borderColor='#6D6AFE';
}
function passwordfocusout(){
    if(!passwordInput.value){
        passwordalert.textContent='비밀번호를 입력해 주세요.';
        passwordInput.style.borderColor='#FF5B56';
    }
    else if(!ispassword.test(passwordInput.value)){
        passwordalert.textContent='비밀번호는 영문, 숫자 조합 8자 이상 입력해 주세요.';
        passwordInput.style.borderColor='#FF5B56';
    }
    else{
        passwordalert.textContent='';
        passwordInput.style.borderColor='#6D6AFE';
        passwordvalid=true;
    }
}
function confirmfocusin(){
    confirmInput.style.borderColor='#6D6AFE';
}
function confirmfocusout(){
    if(passwordInput.value !== confirmInput.value){
        confirmalert.textContent = '비밀번호가 일치하지 않아요.';
        confirmInput.style.borderColor = '#FF5B56';
    }
    else{
        confirmalert.textContent = '';
        confirmInput.style.borderColor = '#6D6AFE';
        confirmvalid=true;
    }
}
export{ idInput,
    passwordInput,
    confirmInput,
    idalert,
    passwordalert,
    confirmalert,
    passwordicon,
    confirmicon,
    corrid,
    corrpassword,
    isemail,
    ispassword,
    successInfo,
    signin,
    signup,
    idfocusin,
    idfocusout,
    passwordfocusin,
    passwordfocusout,
    confirmfocusin,
    confirmfocusout,
}