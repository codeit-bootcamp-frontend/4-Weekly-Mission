...
const ModalButton = styled.button<StringProp>`
  cursor: pointer;
  width: 100%;
  padding: 16px 20px;
  color: #fff;
...
function FolderName() {
  ...
  const [modalFolderContents, setModalFolderContents] = useState<Data>({
    data: [],
  });
  ...
  useEffect(() => {
    folderName.contents.data.forEach(({ id }: { id: number }) => {
      folderLinksData(id)
        .then((data) =>
          setModalFolderContents((prevState) => ({
            ...prevState,
...
function ShareModal({ folderId }: { folderId: FolderId }) {
  //배포한 사이트 필요 시 변경
  ...
  return (
    <ul className="shareContainer">
      <li>
        ...
        >
          <img
            src={`${process.env.PUBLIC_URL}/images/kakaoShare.png`}
            alt="카카오톡으로 공유"
          />
          <span>카카오톡</span>
        </button>
      ...
      <li>
        <button onClick={handleShareToFacebook}>
          <img
            src={`${process.env.PUBLIC_URL}/images/facebookShare.png`}
            alt="페이스북으로 공유"
          />
          <span>페이스북</span>
        </button>
      ...
      <li>
        <button onClick={handleCopyClipBoard}>
          <img
            src={`${process.env.PUBLIC_URL}/images/linkShare.png`}
            alt="링크 복사"
          />
          <span>링크 복사</span>
        </button>
...
function Modal({ name, setOpenModal, buttonColor, buttonName }: Modalprops) {
  const folderName = useRecoilValueLoadable(folderContents);
  const folderLink = useRecoilValue(clickFolder);
  const modalLink = useRecoilValue(contentsLink);
  const folderId = folderName?.contents?.data?.find(
    ({ name }: StringProp) => name === folderLink
  );
