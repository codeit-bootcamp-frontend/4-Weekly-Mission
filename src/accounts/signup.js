...
const passwordCheckErrorMessage = document.querySelector("#passwordcheck-error-message");
...
function validatePasswordCheckInput(passwordcheck) {
  if(passwordCheckInput.value !== passwordInput) {
...
const passwordCheckToggleButton = document.querySelector("#passwordcheck-toggle");
...
const isValidUser =
emailInput.value !== TEST_USER.email && passwordInput.value === passwordCheckInput.value  && emailInput.value !== "" && passwordInput.value !== "";

if (isValidUser) {
  location.href = "/folder";
  return;
}
emailInput.value === TEST_USER.email ? setInputError({ input: emailInput, errorMessage: emailErrorMessage }, "이미 존재하는 이메일입니다.") :
setInputError(
