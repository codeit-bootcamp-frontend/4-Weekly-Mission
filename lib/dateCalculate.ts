export function calculateDateDifference(stringDate: string) {
  const currentTime = new Date();
  const time = new Date(stringDate);

  const timeDifference= Number(currentTime) - Number(time);

  return getDateDifferencesLabel(timeDifference);
}

function getDateDifferencesLabel(timeDifference: number) {
...
function calculateDateType(timeDifference: number) {
