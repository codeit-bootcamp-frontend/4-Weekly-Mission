...
## 계층(아키텍쳐)
- [x] 커스텀 훅을 통해, api 계층과 컴포넌트의 과도한 응집력을 제거한다.
- [ ] 페이지 전환에 대해 고민한다.
...
## 컴포넌트
- [ ] 컴포넌트를 나누는 기준을 세운다.
 - [ ] 특히 하나의 컴포넌트에 너무 많은 조건부 렌더링은 지양한다.
 - [ ] 폴더 컴포넌트를 리팩토링한다.

 # 멘토님 피드백
 ## 설정
 - [ ] 파일명을 대문자, 소문자 변경 시, git Config에 유의한다.
 - [ ] eslint는 eslint만 설치해서 사용하기보다는 plugin, config가 붙은 패키지들을 설치해 규칙을 만든다.
 - [ ] dependencies 패키지와 devDependencies 패키지를 구별한다.
 - [ ] 패키지 버전 앞에 붙어있는 ^(캐럿)에 대해서도 고려한다.